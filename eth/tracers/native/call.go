...
func (f *callFrame) processOutput(_output []byte, err error) {
    _output = common.CopyBytes(_output)
	output := bytesToHex(_output)
	if err == nil {
		f.Output = output
		return
	}
	...
	if !errors.Is(err, vm.ErrExecutionReverted) || len(_output) == 0 {
		return
	}
	f.Output = output
	if len(_output) < 4 {
	...
	if unpacked, err := abi.UnpackRevert(_output); err == nil {
		f.Revertal = unpacked
	}
}
