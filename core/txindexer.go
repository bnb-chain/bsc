...
func (indexer *txIndexer) run(tail *uint64, head uint64, stop chan struct{}, done chan struct{}) {
	...
	if tail == nil {
		...
		rawdb.IndexTransactions(indexer.db, from, head+1, stop)
		return
	}
	...
	if indexer.limit == 0 || head < indexer.limit {
		if *tail > 0 {
			...
			rawdb.IndexTransactions(indexer.db, 0, end, stop)
		}
		return
	}
	...
	if head-indexer.limit+1 < *tail {
		...
		rawdb.IndexTransactions(indexer.db, head-indexer.limit+1, *tail, stop)
	} else {
		// Unindex a part of stale indices and forward index tail to HEAD-limit
		rawdb.UnindexTransactions(indexer.db, *tail, head-indexer.limit+1, stop)
	}
}
...
func (indexer *txIndexer) writeTxIndexTail(ancientLimit uint64) {
	// The tx index tail can only be one of the following two options:
	// * 0: all ancient blocks have been indexed
	// * ancient-limit: the indices of blocks before ancient-limit are ignored
	if tail := rawdb.ReadTxIndexTail(indexer.db); tail == nil {
		if indexer.limit == 0 || ancientLimit <= indexer.limit {
			rawdb.WriteTxIndexTail(indexer.db, 0)
		} else {
			rawdb.WriteTxIndexTail(indexer.db, ancientLimit-indexer.limit)
		}
	}
}

func (indexer *txIndexer) WriteTxIndexByBlock(batch ethdb.Batch, block *types.Block, ancientLimit uint64) {
	if indexer.limit == 0 || ancientLimit <= indexer.limit || block.NumberU64() >= ancientLimit-indexer.limit {
		rawdb.WriteTxLookupEntriesByBlock(batch, block)
	} else if rawdb.ReadTxIndexTail(indexer.db) != nil {
		rawdb.WriteTxLookupEntriesByBlock(batch, block)
	}
}

func (indexer *txIndexer) EnsureTxLookupLimit() {
	limit := indexer.limit
	if stored := rawdb.ReadFastTxLookupLimit(indexer.db); stored == nil {
		rawdb.WriteFastTxLookupLimit(indexer.db, limit)
	} else if *stored != limit {
		indexer.limit = *stored
