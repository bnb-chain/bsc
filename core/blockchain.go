...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine,
	...
	*/
	...
	if bc.pipeline {
		log.Info("blockchain start with pipeline mode1")
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		if statedb.IsPipeLineMode() {
			...
			blockValidationTimer.Update(vtime - (triehash + trieUpdate))    // The time spent on block validation                          // The time spent on block validation
