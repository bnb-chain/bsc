...
func TestIntermediateLeaks(t *testing.T) {
	...
	transRoot, _, err := transState.Commit(false, nil)
	if err != nil {
	...
	finalRoot, _, err := finalState.Commit(false, nil)
	if err != nil {
...
func TestTouchDelete(t *testing.T) {
	...
	root, _, _ := s.state.Commit(false, nil)
...
func TestDeleteCreateRevert(t *testing.T) {
	...
	root, _, _ = state.Commit(true, nil)
	state, _ = New(root, state.db, state.snaps)
...
func TestMissingTrieNodes(t *testing.T) {
	...
	// Modify the state
	state.SetBalance(addr, big.NewInt(2))
	root, _, err := state.Commit(false, nil)
	if err == nil {
