...
//	}
//
//	// Encode the wrap layer
//	if err := rlp.Encode(w, tx.wrapData); err != nil {
//		return err
//	}
//
//	// Encode the inner TxData
//	return rlp.Encode(w, tx.inner)
//
...
func (tx *Transaction) decodeTyped(b []byte) (TxData, TxWrapData, error) {
	...
	case BlobTxType:
		...
		//}
		buf2 := new(bytes.Buffer)
		if err := rlp.Encode(buf2, &testTxData); err != nil {
			return nil, nil, err
		}
		...
		if err := rlp.Encode(buf, &testTxWrapData); err != nil {
			return nil, nil, err
		}
		wrapRlpSize := uint64(buf.Len())

		fmt.Println("dataRlpSize: ", dataRlpSize)
		fmt.Println("wrapRlpSize: ", wrapRlpSize)
...
// decodeTypedMinimal decodes a typed transaction from the canonical format.
...
func NewMessage(from common.Address, to *common.Address, nonce uint64, amount *big.Int, gasLimit uint64, gasPrice, gasFeeCap, gasTipCap, maxFeePerDataGas *big.Int, data []byte, accessList AccessList, isFake bool, dataHashes []common.Hash) Message {
	return Message{
		from:             from,
		to:               to,
		nonce:            nonce,
