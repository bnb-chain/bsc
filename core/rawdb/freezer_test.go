...
func TestFreezerModifyRollback(t *testing.T) {
	...
	f2, err := NewFreezer(dir, "", false, 0, 2049, tables, false)
	if err != nil {
...
func TestFreezerReadonlyValidate(t *testing.T) {
	...
	f, err := NewFreezer(dir, "", false, 0, 2049, tables, false)
	if err != nil {
	...
	// table lengths.
	_, err = NewFreezer(dir, "", true, 0, 2049, tables, false)
	if err == nil {
...
func TestFreezerConcurrentReadonly(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(i int) {
			...
			f, err := NewFreezer(dir, "", true, 0, 2049, tables, false)
			if err == nil {
				fs[i] = f
			} else {
				errs[i] = err
...
func TestFreezer_AdditionTables(t *testing.T) {
	...
	// Open non-readonly freezer and fill individual tables
	// with different amount of data.
	f, err := NewFreezer(dir, "", false, 0, 2049, map[string]bool{"o1": true, "o2": true}, false)
	if err != nil {
		t.Fatal("can't open freezer", err)
	}
	...
	f, err = NewFreezer(dir, "", true, 0, 2049, map[string]bool{"o1": true, "o2": true, "a1": true}, false)
	require.NoError(t, err)
	require.NoError(t, f.Close())

	f, err = NewFreezer(dir, "", false, 0, 2049, map[string]bool{"o1": true, "o2": true, "a1": true}, false)
	require.NoError(t, err)
	frozen, _ := f.Ancients()
...
func TestFreezerSuite(t *testing.T) {
	...
	ancienttest.TestResettableAncientSuite(t, func(kinds []string) ethdb.ResettableAncientStore {
		...
		f, _ := newResettableFreezer(t.TempDir(), "", false, 0, 2048, tables, false)
		return f
	})
}
