...
func fill(n int, env *testEnv) {
	var (
		set      = make(map[string]*trienode.WithPrev)
	...
	for i := 0; i < n; i++ {
		...
		case 2:
			...
			set[string(path)] = trienode.NewWithPrev(common.Hash{}, nil, prev)
		}
	}
	// Add the root node
	...
	env.db.Update(root.Hash, env.lastHash(), map[common.Hash]map[string]*trienode.WithPrev{{}: set})
