...
type ChainConfig struct {
	...
	FourierBlock    *big.Int `json:"fourierBlock,omitempty" toml:",omitempty"`    // fourierBlock switch block (nil = no fork, 0 = already activated)
...
func (c *ChainConfig) IsFourier(num *big.Int) bool {
	return isForked(c.FourierBlock, num)
}

func (c *ChainConfig) IsOnFourier(num *big.Int) bool {
	return configNumEqual(c.FourierBlock, num)
}
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "fourierBlock", block: c.FourierBlock},
	} {
		if lastFork.name != "" {
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.FourierBlock, newcfg.FourierBlock, head) {
		return newCompatError("fourier fork block", c.FourierBlock, newcfg.FourierBlock)
	}
	return nil
}
