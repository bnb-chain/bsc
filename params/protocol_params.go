...
const (
	...
	MinBlocksForBlobRequests           = 4096 * 32 * 12 / 3  // it keeps blob data available for ~18.2 days in local.
	DefaultExtraReserveForBlobRequests = 1 * (24 * 3600) / 3 // it adds more time for expired blobs for some request cases, like expiry blob when remote peer is syncing, default 1 day.
)
