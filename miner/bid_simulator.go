...
func (b *bidSimulator) dialSentryAndBuilders() {
	...
	if b.config.SentryURL != "" {
		sentryCli, err = ethclient.DialOptions(context.Background(), b.config.SentryURL, rpc.WithHTTPClient(client))
		if err != nil {
	...
	for _, v := range b.config.Builders {
		...
		if b.sentryCli != nil {
			...
			builderCli, err = ethclient.DialOptions(context.Background(), v.URL, rpc.WithHTTPClient(client))
			if err != nil {
...
func (b *bidSimulator) AddBuilder(builder common.Address, url string) error {
	...
	if b.sentryCli != nil {
		...
		if url != "" {
			...
			builderCli, err = ethclient.DialOptions(context.Background(), url, rpc.WithHTTPClient(client))
			if err != nil {
