...
func NewStateIterator(triedb *triedb.Database, db ethdb.Database, root common.Hash, cap int) (*StateIterator, error) {
	...
	snapshots, err := snapshot.New(config, db, triedb, root, cap, false)
	if err != nil {
		return nil, err
	}
	return &StateIterator{
