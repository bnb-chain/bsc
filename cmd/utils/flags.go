...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly, disableFreeze bool) ethdb.Database {
	...
	default:
		var (
			...
			chainDBCache   = cache
			chainDBHandles = handles
		)
		if isMultiDB {
			chainDBCache = int(float64(cache*stack.Config().Storage.ChainDB.CacheRatio) / 100)
			chainDBHandles = int(float64(handles*stack.Config().Storage.ChainDB.CacheRatio) / 100)
		}
