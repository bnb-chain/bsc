...
func createNodeConfig(baseConfig gethConfig, ip string, port int, enodes []*enode.Node, index int) gethConfig {
...
func createConfigs(base gethConfig, initDir string, prefix string, ips []string, ports []int, extraEnodes []*enode.Node, connectOneExtraEnodes bool) ([]gethConfig, []*enode.Node, [][]common.Address, error) {
	...
	for i := 0; i < size; i++ {
		...
		configs[i] = createNodeConfig(base, ips[i], ports[i], allEnodes, index)
	}
