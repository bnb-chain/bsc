...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if ctx.IsSet(utils.OverrideB20ActivationAdmin.Name) {
		// Parsed strictly: HexToAddress pads and truncates silently, and this value
		// is seeded into consensus state once and can never be replaced by a
		// transaction, so a typo has to stop the node rather than name a different
		// account. The zero address is refused for the same reason — it cannot
		// originate a call, so it would leave the switch permanently shut.
