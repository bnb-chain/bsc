// summit the evidence of malicious voting
package main
...
var (
	...
	evidenceFlag = cli.StringFlag{
		Name:  "evidence",
		Usage: "params for submitFinalityViolationEvidence in json format; wrapped in single quotes",
	}
)

func init() {
	app = flags.NewApp(gitCommit, gitDate, "a tool for summitting the evidence for malicious voting")
	app.Flags = []cli.Flag{
		senderFlag,
		nodeFlag,
		chainIdFlag,
	...
	app.Action = summitMaliciousVotes
	cli.CommandHelpTemplate = flags.AppHelpTemplate
}

func summitMaliciousVotes(c *cli.Context) {
	// get sender
	...
	if err != nil {
		log.Crit("summitMaliciousVotes:", "error", err)
	}
	var rc *types.Receipt
	for i := 0; i < 180; i++ {
		rc, err = client.TransactionReceipt(context.Background(), tx.Hash())
		if err == nil && rc.Status != 0 {
			log.Info("summitMaliciousVotes: submit evidence success")
			break
		}
		if rc != nil && rc.Status == 0 {
			log.Crit("summitMaliciousVotes: tx failed: ", "error", err, "receipt", rc)
		}
		time.Sleep(100 * time.Millisecond)
	}
	if rc == nil {
		log.Crit("summitMaliciousVotes: submit evidence failed")
	}
}
