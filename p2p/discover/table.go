...
func (tab *Table) filterNode(n *enode.Node) bool {
	...
	entryErr := n.Record().Load(enr.WithEntry("eth", &eth))
	tab.log.Trace("ENR record filter in", "id", n.ID(), "ipAddr", n.IPAddr(), "updPort", n.UDP(), "record", n.Record(), "eth", eth, "entryErr", entryErr)
	return false
}
