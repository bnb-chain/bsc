...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, map[common.Hash]common.Address, error) {
	...
	// Assemble the block
	// nil lane: a simulated block is never sealed, propagated or imported, so it carries no
	// BEP-703 commitment and its uncle slot stays EmptyUncleHash even past Gauss. Verified
	// on a live post-Gauss chain in both validation modes - nothing here decodes the slot,
	// because simulation does not run core.Process. The only consequence is that the block
	// this returns has a shape the chain would reject, which matters solely to a caller
	// that treats eth_simulateV1 output as chain-shaped.
