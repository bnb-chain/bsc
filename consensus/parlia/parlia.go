...
func (p *Parlia) distributeFinalityReward(chain consensus.ChainHeaderReader, state vm.StateDB, header *types.Header,
	...
	usedGas *uint64, mining bool, signTx bool, tracer *tracing.Hooks) error {
	...
	return p.applyTransaction(msg, state, header, cx, txs, receipts, systemTxs, usedGas, mining, signTx, tracer)
}
...
func (p *Parlia) distributeIncoming(val common.Address, state vm.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool, signTx bool, tracer *tracing.Hooks) error {
	...
	if doDistributeSysReward {
		...
		if rewards.Cmp(common.U2560) > 0 {
			...
			err := p.distributeToSystem(rewards.ToBig(), state, header, chain, txs, receipts, receivedTxs, usedGas, mining, signTx, tracer)
			if err != nil {
				return err
			}
	...
	return p.distributeToValidator(balance.ToBig(), val, state, header, chain, txs, receipts, receivedTxs, usedGas, mining, signTx, tracer)
}
...
func (p *Parlia) applyTransaction(
	...
	receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool, signTx bool,
